...
export const DesktopTableView = ({ data, columns, actions, sort, onToggleSort }) => {
  ...
  return (
    <div className="flex-1 overflow-auto rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm">
      <table className="w-full text-sm">
        ...
        <tbody>
          {data.map((row, i) => (
            <tr
              key={row.id ?? i}
              className="border-t border-gray-100 dark:border-slate-700 hover:bg-blue-50 dark:hover:bg-slate-700 transition-colors animate-row"
              style={{ animationDelay: `${i * 40}ms` }}
            >
              {columns.map((col) => (
                <td
                  key={col.key}
                  className="px-4 py-3 text-gray-700 dark:text-slate-200 whitespace-nowrap"
                  style={{ textAlign: col.align || "left" }}
                >
