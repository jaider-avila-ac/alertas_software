...
import { crearDocente, actualizarDocente } from "../../services/docentes.service";
...
export default function DocenteFormPage() {
  ...
  const inicial   = esEdicion && state?.docente
    ? {
        nombres:   state.docente.nombres   ?? "",
        apellidos: state.docente.apellidos ?? "",
        documento: state.docente.documento ?? "",
        genero:    state.docente.genero    ?? "",
        direccion: state.docente.direccion ?? "",
        correo:    state.docente.correo    ?? "",
        telefono:  state.docente.telefono  ?? "",
      }
    : FORM_VACIO;
  ...
  const handleSubmit = async e => {
    ...
    setFormError("");
    try {
      esEdicion ? await actualizarDocente(id, form) : await crearDocente(form);
      navigate(`/${slug}/docentes`);
    } catch (err) {
  ...
  return (
    <div className="p-2">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button
          onClick={() => navigate(`/${slug}/docentes`)}
          className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors"
        >
          <ArrowLeft size={18} />
        </button>
        <p className="text-sm text-gray-400">
          {esEdicion ? "Modifica los datos del docente" : "Completa los datos para registrar un docente"}
        </p>
      </div>
